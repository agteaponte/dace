...
function item_Mt_data(d) {
  ...
  return `<div class="record-item" style="border-left-color:#7c3aed">
    ...
    <div class="record-foot">
      ...
      <button class="btn-edit" onclick="editarMantenimiento('${d._id}')">âœï¸ Editar</button>
...
function item_Q1_data(d) {
  return `<div class="record-item">
    ...
    <div class="record-foot">
      ${bEstatus(d.estatus)}
      <button class="btn-edit" onclick="editarQ1('${d._id}')">âœï¸ Editar</button>
...
function item_Q3_data(d) {
  return `<div class="record-item gold">
    ...
    <div class="record-foot">
      ${bEstatus(d.estatus)}
      <button class="btn-edit" onclick="editarQ3('${d._id}')">âœï¸ Editar</button>
...
function item_Caso_data(d) {
  ...
  return `<div class="record-item ${cls}">
    ...
    <div class="record-foot">
      ...
      <button class="btn-edit" onclick="editarCaso('${d._id}')">âœï¸ Editar</button>
...
function item_Maestro_data(d) {
  return `<div class="record-item gray">
    ...
    <div class="record-foot">
      <button class="btn-edit" onclick="editarMaestro('${d._id}')">âœï¸ Editar</button>
...
function renderGeneradores(docs) {
  ...
  const condIcon = { 'Ã“ptimo':'<i class="ph-bold ph-check"></i>', 'Regular':'âš ï¸', 'Deficiente':'<i class="ph-bold ph-x"></i>', 'Fuera de Servicio':'ðŸš«' };
  const tipoColor = {
  ...
  el.innerHTML = docs.map(d => `
    <div class="record-item" style="border-left-color:${tipoColor[d.tipo]||'#15803d'}">
      ...
      ${d.nivelAntes!=null ? `<p class="record-body">â›½ Nivel: <strong>${d.nivelAntes}%</strong> â†’ <strong>${d.nivelDespues||'â€”'}%</strong>${d.galones ? ` Â· ${d.galones} gal` : ''}${d.costo ? ` Â· $${d.costo}` : ''}</p>` : ''}
      ${d.horas ? `<p class="record-body">â± Horas operaciÃ³n: ${d.horas}</p>` : ''}
...
async function guardarAgenda() {
  ...
  if (!titulo || !fecha) { showToast('âš ï¸ TÃ­tulo y fecha son requeridos', '#92400e'); return; }
  try {
...
function renderAgenda(docs) {
  ...
  if (!docs.length) {
    el.innerHTML = '<div class="empty-state"><span class="empty-icon">ðŸ“…</span><p>No hay tareas registradas</p></div>';
    return;
  }
  // Ordenar por fecha
  ...
  el.innerHTML = sorted.map(d => {
    ...
    return `<div class="record-item" style="border-left-color:${borde}">
      ...
      <div class="record-foot">
        ...
        <button class="btn-del" onclick="completarTarea('${d._id}','${d.estatus}')">${d.estatus==='Completado'?'â†©ï¸ Reabrir':'<i class="ph-bold ph-check"></i> Completar'}</button>
...
async function completarTarea(id, estatusActual) {
  ...
  try {
    ...
    showToast(nuevoEstatus === 'Completado' ? '<i class="ph-bold ph-check"></i> Tarea completada' : 'â†©ï¸ Tarea reabierta', '#166534');
  } catch(e) { showToast('<i class="ph-bold ph-x"></i> Error: ' + e.message, '#dc2626'); }
}
...
function renderDirectorio(docs) {
  ...
  if (!docs.length) {
    el.innerHTML = '<div class="empty-state"><span class="empty-icon">ðŸ‘¥</span><p>No hay contactos registrados</p></div>';
    return;
  }
  el.innerHTML = docs.map(d => `
    <div class="record-item" style="border-left-color:#0f766e">
      ...
      ${d.telefono ? `<p class="record-body">ðŸ“ž ${d.telefono}${d.extension?' ext.'+d.extension:''}</p>` : ''}
      ${d.email    ? `<p class="record-body">âœ‰ï¸ ${d.email}</p>` : ''}
      ${d.notas    ? `<p class="record-body" style="color:#94a3b8;font-style:italic">${d.notas}</p>` : ''}
      <div class="record-foot">
...
function renderArchivo(docs) {
  ...
  if (!docs.length) {
    el.innerHTML = '<div class="empty-state"><span class="empty-icon">ðŸ—ƒï¸</span><p>No hay documentos registrados</p></div>';
    return;
  }
  el.innerHTML = docs.map(d => `
    <div class="record-item" style="border-left-color:#b45309">
      ...
      ${d.fecha       ? `<p class="record-body">ðŸ“… ${d.fecha}</p>` : ''}
...
function renderJedi(docs) {
  ...
  if (!docs.length) {
    el.innerHTML = '<div class="empty-state"><span class="empty-icon">ðŸ–¥ï¸</span><p>No hay solicitudes registradas</p></div>';
    return;
  }
  const colorEstatus = {
  ...
  const iconEstatus = {
    ...
    'En asignaciÃ³n de Presupuesto':        'ðŸ’°',
    'En proceso de compra':                'ðŸ”„',
  ...
  el.innerHTML = docs.map(d => {
    ...
    <div class="record-item" style="border-left-color:${colorEstatus[d.estatus]||'#1d4ed8'}">
      ...
      <div class="record-foot">
        ...
        ${diasAbierto !== null ? `<span style="font-size:11px;font-weight:800;color:${colorDias}">ðŸ“… ${diasAbierto} dÃ­as</span>` : ''}
      ...
      <div class="record-foot" style="margin-top:8px">
        <button class="btn-edit" onclick="editarJedi('${d._id}')">âœï¸ Editar</button>
...
async function guardarQ1() {
  ...
  if (!unidad || !desc) { showToast('âš ï¸ Unidad y descripciÃ³n son requeridos', '#92400e'); return; }
...
async function guardarQ3() {
  ...
  if (!lugar || !hall) { showToast('âš ï¸ Lugar y hallazgos son requeridos', '#92400e'); return; }
  ...
  try {
    if (_editando.col === 'dace_q137_3' && _editando.id) {
      ...
      if(btn) { btn.style.background=''; btn.innerHTML='ðŸ’¾ GUARDAR INSPECCIÃ“N'; }
    } else {
      ...
      datos.fotoUrl = fotoUrl || null;
      await docRef.set(datos);
      showToast('<i class="ph-bold ph-check"></i> InspecciÃ³n ' + datos.numero + ' guardada', '#166534');
    }
...
function pdfHeader(doc, titulo) {
  ...
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  doc.text('Negociado de la PolicÃ­a de Puerto Rico', W / 2, 20, { align: 'center' });
...
async function exportarPDF(modulo) {
  ...
  try {
    if (modulo === 'q137_1') {
      titulo = 'Registro de Ã“rdenes de Trabajo PPR-137.1';
      ...
      if (snap.empty) {
        doc.setTextColor(150,150,150);
        doc.setFontSize(10);
        doc.text('No hay Ã³rdenes registradas.', W/2, y+10, {align:'center'});
      } else {
        for (let i = 0; i < snap.docs.length; i++) {
          ...
          doc.setFontSize(9);
          doc.setFont('helvetica','bold');
          doc.text(`#${i+1}  ${d.numero||'â€”'}`, 13, y+5);
          doc.setFont('helvetica','normal');
          ...
          const campos = [
            ...
            ['DescripciÃ³n', d.descripcion],
            ['LocalizaciÃ³n', [d.seccion, d.division, d.distrito].filter(Boolean).join(' Â· ')],
            ['Estatus', d.estatus],
            ['Observaciones', d.observaciones],
          ];
          campos.forEach(([label, val]) => {
          ...
          if (d.fotoUrl) {
            try {
              ...
              doc.setFontSize(7);
              doc.setFont('helvetica','bold');
              doc.text('Evidencia FotogrÃ¡fica:', 13, y);
              y += 4;
              const imgData = await cargarImagenComoBase64(d.fotoUrl);
              if (imgData) {
      ...
      if (snap.empty) {
        ...
        for (let i = 0; i < snap.docs.length; i++) {
          ...
          if (d.inspeccion) {
            const secNombres = {
              ext:'Exterior', int:'Interior', com:'Comedor', adm:'AdministraciÃ³n',
              bib:'Biblioteca', aca:'AcadÃ©mico', ban:'BaÃ±os', tec:'Techos',
              can:'Cancha', seg:'Seguridad', ele:'Electricidad', dor:'Dormitorios'
            };
            Object.keys(d.inspeccion).forEach(sec => {
              ...
              doc.text(`â–¶ ${secNombres[sec]||sec}:`, 14, y+3);
              y += 8;
              items.forEach(it => {
                y = checkPage(doc, y, 7);
                doc.setFont('helvetica','normal');
                doc.setTextColor(60,60,60);
                const tipo = it.estado === 'R' ? '[Reemplazo]' : '[ReparaciÃ³n]';
                const txt  = `${tipo} ${it.item}${it.observaciones?' â€” '+it.observaciones:''}`;
...
async function exportarPanelPDF() {
  ...
  try {
    ...
    s2.docs.slice(0,5).forEach((docSnap) => {
      ...
      doc.setTextColor(100,100,100);
      doc.setFontSize(7);
      doc.text(`${d.lugar||''} Â· ${d.tipo||''}`, 50, y+4);
      doc.text(d.estatus||'', 50, y+8);
      doc.text(`${d.fecha||''}`, W-13, y+6, {align:'right'});
      y += 12;
    });
    ...
    doc.setTextColor(150,150,150);
    doc.setFontSize(7);
    doc.text('DACE Arecibo â€” NPPR â€” Documento oficial',
      W/2, doc.internal.pageSize.getHeight()-3, {align:'center'});
    ...
    doc.setTextColor(...azul); doc.setFontSize(7); doc.setFont('helvetica','bold');
    doc.text('Agte. Jose C. Aponte Cancel Â· Placa 31093', 10, doc.internal.pageSize.getHeight()-34);
    doc.setFont('helvetica','normal'); doc.setTextColor(100,100,100);
...
function initListeners() {
  ...
  db.collection('dace_q137_1')
    ...
    .onSnapshot(snap => {
      ...
      aplicarFiltros('q1');
      feedCache.q1 = snap.docs.slice(0,3).map(d => feedRow('<i class="ph-fill ph-scales"></i>', d.data().numero||'Incidente', `137.1 Â· ${d.data().fecha||''} Â· ${d.data().estatus||''}`));
      renderFeed();
  ...
  db.collection('dace_q137_3')
    ...
    .onSnapshot(snap => {
      ...
      aplicarFiltros('q3');
      feedCache.q3 = snap.docs.slice(0,3).map(d => feedRow('ðŸ—ï¸', d.data().numero||'InspecciÃ³n', `137.3 Â· ${d.data().lugar||''} Â· ${d.data().estatus||''}`));
      renderFeed();
  ...
  db.collection('dace_maestro')
    ...
    .onSnapshot(snap => {
      ...
      aplicarFiltros('rm');
      feedCache.maestro = snap.docs.slice(0,2).map(d => feedRow('ðŸ“', d.data().tipo||'Entrada', trunc(d.data().descripcion||'', 55)));
      renderFeed();
...
async function guardarCaso() {
  ...
  try {
    if (_editando.col === 'dace_casos' && _editando.id) {
      ...
      if (!num) { showToast('âš ï¸ NÃºmero es requerido', '#92400e'); return; }
      datos.createdAt = firebase.firestore.FieldValue.serverTimestamp();
...
async function guardarMantenimiento() {
  ...
  if (!lugar || !desc) { showToast('âš ï¸ Lugar y descripciÃ³n son requeridos', '#92400e'); return; }
  const datos = {
...
async function imprimirIndividualPDF(col, id) {
  ...
  try {
    ...
    if(col === 'dace_q137_1') {
      ...
      campos = [
        ['NÃºmero', d.numero], ['Fecha', `${d.fecha||''} ${d.hora||''}`],
        ['Solicitante', d.solicitante], ['Unidad', d.unidad],
        ['Tipos de Trabajo', d.trabajos?.join(', ')], ['DescripciÃ³n', d.descripcion],
        ['Estatus', d.estatus], ['Observaciones', d.observaciones]
      ];
    } else if(col === 'dace_q137_3') {
      titulo = 'InspecciÃ³n PPR-137.3 (Indiv.)';
      campos = [
        ['NÃºmero', d.numero], ['Fecha', `${d.fecha||''} ${d.hora||''}`],
