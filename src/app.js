...
function renderGeneradores(docs) {
  ...
  if (!docs.length) {
    el.innerHTML = '<div class="empty-state"><span class="empty-icon"><i class="ph-bold ph-x"></i>9</span><p>No hay registros en la bitácora</p></div>';
    return;
  }
...
async function guardarInsp312_2() {
  ...
  try {
    if (_editando312.id) {
      ...
      if (btn) btn.innerHTML = '<i class="ph-bold ph-x"></i>� REGISTRAR INSPECCI�N PPR-312.2';
    } else {
...
function renderInsp312_2(docs) {
  ...
  if (!docs.length) {
    el.innerHTML = '<div class="empty-state"><span class="empty-icon"><i class="ph-bold ph-x"></i>�</span><p>No hay inspecciones PPR-312.2 registradas</p></div>';
    return;
  }
...
function editarInsp312_2(id) {
  ...
  const btn = document.querySelector('#gen_sec_ppr_registro .btn-save');
  if (btn) btn.innerHTML = '<i class="ph-bold ph-x"></i>� GUARDAR CAMBIOS PPR-312.2';
...
async function editarJedi(id) {
  try {
    ...
    setTimeout(() => {
      ...
      if(btn) { btn.style.background='#b45309'; btn.innerHTML='<i class="ph-bold ph-x"></i>� ACTUALIZAR SOLICITUD JEDI'; }
      showToast('<i class="ph-fill ph-pencil-simple"></i> Modo edición � modifica y toca ACTUALIZAR', '#b45309');
    }, 300);
  } catch(e) { showToast('<i class="ph-bold ph-x"></i> Error: ' + e.message, '#dc2626'); }
}
...
async function guardarQ1() {
  ...
  try {
    if (_editando.col === 'dace_q137_1' && _editando.id) {
      ...
      if(btn) { btn.style.background=''; btn.innerHTML='<i class="ph-bold ph-x"></i>� GUARDAR ORDEN DE TRABAJO'; }
    } else {
...
function loginFirebase() {
  ...
  if (!email || !pass) { errEl.textContent = '⚠️ Completa correo y contraseña.'; return; }
...
try {
  ...
  firebase.auth().onAuthStateChanged(user => {
    if (user) {
      ...
      document.getElementById('lockScreen').style.display  = 'none';
      setFbStatus('error', '<i class="ph-bold ph-x"></i> Sesión no iniciada � Inicia sesión para continuar');
    }
  });
...
function initListeners() {
  ...
  db.collection('dace_q137_3')
    ...
    .onSnapshot(snap => {
      ...
      aplicarFiltros('q3');
      feedCache.q3 = snap.docs.slice(0,3).map(d => feedRow('<i class="ph-fill ph-buildings"></i>', d.data().numero||'Inspección', `137.3 · ${d.data().lugar||''} · ${d.data().estatus||''}`, 'q137_3', d.data().numero || d.id));
      renderFeed();
  ...
  db.collection('dace_maestro')
    ...
    .onSnapshot(snap => {
      ...
      aplicarFiltros('rm');
      feedCache.maestro = snap.docs.slice(0,2).map(d => feedRow('<i class="ph-fill ph-folder-open"></i>', d.data().tipo||'Entrada', trunc(d.data().descripcion||'', 55), 'maestro', d.data().tipo || d.id));
      renderFeed();
...
function renderFeed() {
  ...
  if (!all.length) {
    el.innerHTML = '<div class="empty-state"><span class="empty-icon"><i class="ph-bold ph-x"></i>�</span><p>Sin actividad registrada aún</p></div>';
    return;
  }
...
function item_Q1(doc) {
  ...
  return `<div class="record-item">
    ...
    <div class="record-foot">
      ${bEstatus(d.estatus)}
      <button class="btn-edit" onclick="editarQ1('${id}')"><i class="ph-fill ph-pencil-simple"></i> Editar</button>
...
function item_Q3(doc) {
  ...
  return `<div class="record-item gold">
    ...
    <div class="record-foot">
      ${bEstatus(d.estatus)}
      <button class="btn-edit" onclick="editarQ3('${id}')"><i class="ph-fill ph-pencil-simple"></i> Editar</button>
...
function item_Caso(doc) {
  ...
  return `<div class="record-item ${cls}">
    ...
    <div class="record-foot">
      ${bPrio(d.prioridad)} ${bEstatus(d.estatus)}
      <button class="btn-edit" onclick="editarCaso('${id}')"><i class="ph-fill ph-pencil-simple"></i> Editar</button>
...
function item_Maestro(doc) {
  ...
  return `<div class="record-item gray">
    ...
    ${d.notas ? `<p class="record-body" style="color:#94a3b8;font-style:italic">${d.notas}</p>` : ''}
    <div class="record-foot">
      <button class="btn-edit" onclick="editarMaestro('${id}')"><i class="ph-fill ph-pencil-simple"></i> Editar</button>
...
function modoEdicion(col, id, labelBtn) {
  ...
  btns.forEach(b => {
    if (b.textContent.includes('GUARDAR') || b.textContent.includes('ACTUALIZAR')) {
      ...
      b.innerHTML = '<i class="ph-bold ph-x"></i>� ACTUALIZAR ' + labelBtn;
    }
  });
  showToast('<i class="ph-fill ph-pencil-simple"></i> Modo edición activo � modifica y toca ACTUALIZAR', '#b45309');
  window.scrollTo(0, 0);
}
...
async function editarQ1(id) {
  try {
    ...
    setTimeout(() => {
      ...
      const btn = document.querySelector('#q137_1 .btn-save');
      if(btn) { btn.style.background='#b45309'; btn.innerHTML='<i class="ph-bold ph-x"></i>� ACTUALIZAR ORDEN'; }
    }, 300);
  } catch(e) { showToast('<i class="ph-bold ph-x"></i> Error: ' + e.message, '#dc2626'); }
}
...
async function editarQ3(id) {
  try {
    ...
    setTimeout(() => {
      ...
      if(btn) { btn.style.background='#b45309'; btn.innerHTML='<i class="ph-bold ph-x"></i>� ACTUALIZAR INSPECCI�N'; }
      showToast('<i class="ph-fill ph-pencil-simple"></i> Modo edición activo � modifica y toca ACTUALIZAR', '#b45309');
    }, 300);
  } catch(e) { showToast('<i class="ph-bold ph-x"></i> Error: ' + e.message, '#dc2626'); }
}
...
async function editarMantenimiento(id) {
  try {
    ...
    setTimeout(() => {
      ...
      if(btn) { btn.style.background='#b45309'; btn.innerHTML='<i class="ph-bold ph-x"></i>� ACTUALIZAR MANTENIMIENTO'; }
      showToast('<i class="ph-fill ph-pencil-simple"></i> Modo edición � modifica y toca ACTUALIZAR', '#b45309');
    }, 300);
  } catch(e) { showToast('<i class="ph-bold ph-x"></i> Error: ' + e.message, '#dc2626'); }
}
...
async function editarCaso(id) {
  try {
    ...
    setTimeout(() => {
      ...
      if(btn) { btn.style.background='#b45309'; btn.innerHTML='<i class="ph-bold ph-x"></i>� ACTUALIZAR CASO'; }
    }, 300);
  } catch(e) { showToast('<i class="ph-bold ph-x"></i> Error: ' + e.message, '#dc2626'); }
}
...
async function editarMaestro(id) {
  try {
    ...
    setTimeout(() => {
      ...
      if(btn) { btn.style.background='#b45309'; btn.innerHTML='<i class="ph-bold ph-x"></i>� ACTUALIZAR ENTRADA'; }
    }, 300);
  } catch(e) { showToast('<i class="ph-bold ph-x"></i> Error: ' + e.message, '#dc2626'); }
}
...
async function guardarCaso() {
  ...
  try {
    if (_editando.col === 'dace_casos' && _editando.id) {
      ...
      _editando = { col: null, id: null };
      const btn = document.querySelector('#casos .btn-save');
      if(btn) { btn.style.background=''; btn.innerHTML='<i class="ph-bold ph-x"></i>� GUARDAR CASO'; }
    } else {
...
async function guardarMaestro() {
  ...
  try {
    if (_editando.col === 'dace_maestro' && _editando.id) {
      ...
      _editando = { col: null, id: null };
      const btn = document.querySelector('#maestro .btn-save');
      if(btn) { btn.style.background=''; btn.innerHTML='<i class="ph-bold ph-x"></i>� GUARDAR ENTRADA'; }
    } else {
      datos.createdAt = firebase.firestore.FieldValue.serverTimestamp();
...
function cargarTrabajosRealizados() {
  ...
  Promise.all([
    ...
    if(!docs.length) {
      el.innerHTML = '<div class="empty-state"><span class="empty-icon"><i class="ph-bold ph-x"></i>�</span><p>No hay trabajos completados aún</p></div>';
      return;
    }
...
async function importarConversacionesChatGPT() {
  ...
  reader.onload = async (e) => {
    try {
      ...
      statusEl.textContent = '<i class="ph-bold ph-x"></i>� Buscando coincidencias DACE...';
      let importados = 0;
